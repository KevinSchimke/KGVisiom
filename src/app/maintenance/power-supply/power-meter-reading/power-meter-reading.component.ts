...
@Component({
  selector: 'app-power-meter-reading',
  standalone: true,
  templateUrl: './power-meter-reading.component.html',
  styleUrl: './power-meter-reading.component.scss',
...
export class PowerMeterReadingComponent {
...
constructor(private fb: FormBuilder, private snackBar: MatSnackBar, private http: HttpClient) {
  ...
  this.readingForm.get('parcelNumber')?.valueChanges
    ...
    .subscribe(parcelNumber => {
      if (parcelNumber > 0) {
        this.http.get<{ electrometerID: string | null }>(`https://backend.kgv.local:8443/api/power/active-meter?parcelNumber=${parcelNumber}`)
          .subscribe({
            next: (result) => {
              this.meterNumberDisplay = result.electrometerID || 'Kein Zähler';
            },
            error: () => {
...
saveReading() {
  if (this.readingForm.valid) {
    ...
    this.http.post('https://backend.kgv.local:8443/api/power/readings', payload)
      .subscribe({
        next: () => {
